<x-layouts.admin>
    <x-slot name="title">{{ trans('general.title.new', ['type' => trans('oauth.scope')]) }}</x-slot>

    <x-slot name="favorite"
        title="{{ trans('oauth.scopes.title') }}"
        icon="label"
        route="settings.oauth.scopes.create"
    ></x-slot>

    <x-slot name="content">
        <x-form.container>
            <x-form id="oauth-scope" route="settings.oauth.scopes.store">
                <x-form.section>
                    <x-slot name="head">
                        <x-form.section.head 
